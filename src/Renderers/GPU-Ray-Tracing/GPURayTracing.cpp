...
#include "SceneCreation/GeometryFactory.h"
...
namespace Rutile {
    ...
    void GPURayTracing::Render(RenderTarget& target, const Camera& camera) {
        ...
        if (!m_CreatedAccumulationBuffer) {
            ...
            m_CreatedAccumulationBuffer = true;
        }
        ...
        m_RayTracingShader->SetInt("screenWidth", target.GetSize().x);
        m_RayTracingShader->SetInt("screenHeight", target.GetSize().y);
        ...
        // Read from accumulation framebuffer, divide by frame count, and render to target
        target.Bind();
    ...
    void GPURayTracing::ResetAccumulatedPixelData(glm::ivec2 screenSize) {
        m_FrameCount = 0;
    ...
    void GPURayTracing::CreateAndUploadBVHAndMeshAndObjectBuffers() {
        ...
        for (size_t i = 0; i < m_Scene.objects.size(); ++i) {
            ...
            for (size_t i = 0; i < mesh.indices.size(); i += 3) {
                const Vertex v1 = mesh.vertices[mesh.indices[i + 0]];
